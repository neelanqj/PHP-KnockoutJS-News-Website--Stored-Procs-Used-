...
class UserService
{
	...
	public function getEmailAuthentication() {
			...
			// this is what will happen if the forum has been submitted.
			$from = "noreply@rhanthos.ca";
			
			// Location of administrator's email.
			$to = $this->_email;
			$subject = "RhanthoS Email Authentication Code";
			$comments = "This is your email authentication code: ".$authenticationcode."
Enter your code at the following link in order to gain the ability to reset your password. 
http://rhanthos.ca/apps/view/forgotpassword.view.php";
	...
	public function generateActivationCode() {
			...
			$email = $this->_email;
			$comments = 'This is your verification code: '.$vcode.'
Enter it on the verification page:
http://rhanthos.ca/apps/view/activate.view.php

along with your email address in order to gain access to your account or use the following link. Below is a link:        
http://rhanthos.ca/apps/view/activate.view.php?email='.$email.'&code='.$vcode.'&activate=1';
			
			$to = $email;
			
			$subject = "RhanthoS Login Activation";

			$header = "From: verify@rhanthos.ca\r\nReply-To: verify@rhanthos.ca\r\n";
			$message = "$comments";
    ...
    {	
		...
		echo "<script>setTimeout(location.href='mainall.view.php',3000)</script>";
		return true;
    }
	
	
	public function trackEm() {
	    
		$stmt = $this->_db->prepare("CALL TRACKER (?)");
		$stmt->bindParam(1, $this->_ipaddress, PDO::PARAM_STR, 200);
